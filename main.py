...
OUTPUT_LAS = "Ground points completed.las"
SELECTED_TRIAL_ID = 1
MODEL_DIR = "models"
MAX_HOLE_SIZE = 80.0
VISUALIZE = True
MAX_FILL_POINTS = 200000
CPU_WORKERS = 3

# 🔥 关键：补点密度
STEP = 1.0        # 越小越密，你的点间距≈6m，2.5 效果很自然
JITTER = 1.0      # 随机抖动，越大越自然
...
# 🔥 快速自然采样：网格 + 随机抖动
# -------------------------------
def fast_natural_sampling(x_min, x_max, y_min, y_max, step, jitter):
    ...
    # 加随机抖动，破坏规整感
    xy += np.random.uniform(-jitter, jitter, size=xy.shape)
    return xy
...
def main():
    ...
    hole_xy = fast_natural_sampling(x_min, x_max, y_min, y_max, STEP, JITTER)
    print(f"   候选补点：{len(hole_xy):,}")

    if MAX_FILL_POINTS and len(hole_xy) > MAX_FILL_POINTS:
        hole_xy = hole_xy[np.random.choice(len(hole_xy), MAX_FILL_POINTS, replace=False)]
        print(f"   采样至：{len(hole_xy):,}")
    ...
    if VISUALIZE and len(hole_xy):
        ...
        plt.title(f'补点预览  数量={len(hole_xy)}')
        plt.axis('equal')
        plt.tight_layout()
